...
typedef enum
{
    SpeedSlow, SpeedNormal, SpeedFast, SpeedTurbo,
} motionspeed_e;
...
typedef enum
{
    floor_special, ceiling_special, lighting_special,
} special_e;

//jff 3/15/98 pure texture/type change for better generalized support
typedef enum
{
    trigChangeOnly, numChangeOnly,
} change_e;
...
typedef enum
{
    elevateUp, elevateDown, elevateCurrent,
} elevator_e;
...
{
    return (sector_t*) getLongPtr(p_fm->sector_sptr);
}
...
enum
{
    p_push, p_pull, p_wind, p_current,
};
typedef struct
{
    thinker_t thinker;   // Thinker structure for Pusher
    mobj_t *source;      // Point source if point pusher
    short x_mag;           // X Strength
    short y_mag;           // Y Strength
    short magnitude;       // Vector strength for point pusher
    short radius;          // Effective radius for point pusher
    short x;               // X of point source if point pusher
    short y;               // Y of point source if point pusher
    short affectee;        // Number of affected sector
    byte type;
} pusher_t;
...
int P_CheckTag(const line_t *line); // jff 2/27/98
...
int
EV_DoElevator(const line_t *line, elevator_e type);
...
int EV_VerticalDoor(const line_t *line, mobj_t *thing);
...
int EV_DoPlat(const line_t *line, plattype_e type, int amount);
...
int EV_DoGenStairs(const line_t *line);

int EV_DoGenCrusher(const line_t *line);

int EV_DoGenDoor(const line_t *line);

int EV_DoGenLockedDoor(const line_t *line);
