...
// Game events info
    byte special_event; // Event triggered by local player, to send
...
// 2021-08-07 next-hack: to support full DOOM we need uint16_t and not only bytes. We still have ram
    uint16_t *linesChangeableTextureIndex; // 2021-03-13: forgot that switches have changeable textures...
...
//******************************************************************************
    ...
    short firstflat, lastflat, numflats;
...
// regular wall
//
    drawseg_t drawsegs[MAXDRAWSEGS]; // about 4.6k
    short *lastopening;
...
//spriteframe_t sprtemp[MAX_SPRITE_FRAMES];
    short maxframe;		// nh
    vissprite_t vissprites[MAXVISSPRITES]; // size: 40 *  MAXVISSPRITES
...
// Each screen is [SCREENWIDTH*SCREENHEIGHT]; // 2021-02-13 next-hack: actually no, we use a quarter of the buffer, to save ram
...
const unsigned char* current_palette;
const unsigned char* palette_lump;
int newpal;
