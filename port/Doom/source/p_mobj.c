 ...
 *                 mobj_t structures, for unmovable objects.
 *
 *-----------------------------------------------------------------------------*/
...
void P_XYMovement(mobj_t *mo)
{
     ...
     */
    if ((mo->flags & MF_CORPSE ) && (mo->momx > FRACUNIT / 4 || mo->momx < -FRACUNIT / 4 || mo->momy > FRACUNIT / 4 || mo->momy < -FRACUNIT / 4) && mo->floorz != getMobjSubsector(mo)->sector->floorheight)
...
{
    if (mo->flags & MF_STATIC)
        printf("P_ZMovement static\r\n");
    // check for smooth step up

    if (mo->player_sptr && getMobjPlayer(mo)->mo == mo && // killough 5/12/98: exclude voodoo dolls
    mo->z < mo->floorz)
    {
        getMobjPlayer(mo)->viewheight -= mo->floorz - mo->z;
    ...
    if ((mo->flags & MF_FLOAT ) && getTarget(mo))

        // float down towards target if too close

        if (!((mo->flags ^ MF_FLOAT ) & (MF_FLOAT | MF_SKULLFLY | MF_INFLOAT )) && getTarget(mo)) /* killough 11/98: simplify */
        {
            fixed_t delta;
            if (P_AproxDistance(mo->x - getTarget(mo)->x, mo->y - getTarget(mo)->y) < D_abs(delta = getTarget(mo)->z + (getMobjHeight(mo) >> 1) - mo->z) * 3)
    ...
    if (mo->z <= mo->floorz)
    {
        ...
        if (mo->momz < 0)
        {
            if (mo->player_sptr && /* killough 5/12/98: exclude voodoo dolls */
            getMobjPlayer(mo)->mo == mo && mo->momz < -GRAVITY * 8)
            {
                // Squat down.
                ...
                getMobjPlayer(mo)->deltaviewheight = mo->momz >> 3;
    ...
    if (mo->z + getMobjHeight(mo) > mo->ceilingz)
    {
        /* cph 2001/04/15 -
        ...
        mo->z = mo->ceilingz - getMobjHeight(mo);
...
void P_NightmareRespawn(mobj_t *mobj)
{
    ...
    mo = P_SpawnMobj(mobj->x, mobj->y, getMobjSubsector(mobj)->sector->floorheight, MT_TFOG);
...
//
#include "z_bmalloc.h"
IMPLEMENT_BLOCK_MEMORY_ALLOC_ZONE(mobjzone, ((sizeof(mobj_t) + 3) / 4) * 4, PU_LEVEL, 16, "Mobj");
IMPLEMENT_BLOCK_MEMORY_ALLOC_ZONE(static_mobjzone, ((sizeof(static_mobj_t) + 3) / 4) * 4, PU_LEVEL, 16, "Mobj");
...
mobj_t* P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
    ...
    mobj->floorz = getMobjSubsector(mobj)->sector->floorheight;
    mobj->ceilingz = getMobjSubsector(mobj)->sector->ceilingheight;

    mobj->z = z == ONFLOORZ ? mobj->floorz :
              z == ONCEILINGZ ? mobj->ceilingz - getMobjHeight(mobj) : z;
...
// P_SpawnStaticMobj
//
static_mobj_t* P_SpawnStaticMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
    static_mobj_t *mobj;
    const state_t *st;
    const mobjinfo_t *info;
...
//   mobj->z = z == ONFLOORZ ? mobj->floorz : z == ONCEILINGZ ?    mobj->ceilingz - getMobjHeight(mobj) : z;
    mobj->z =
            z == ONFLOORZ ? getMobjSubsector((mobj_t*) mobj)->sector->floorheight :
            z == ONCEILINGZ ? getMobjSubsector((mobj_t*) mobj)->sector->ceilingheight - getMobjHeight((mobj_t*) mobj) : z;

    mobj->thinker.function = P_ThinkerFunctionForType(type, (mobj_t*) mobj);
...
void P_SpawnPlayer(int n, const mapthing_t *mthing)
{
    ...
    mobj->player_sptr = getShortPtr(p);
...
void P_SpawnPlayerMissile(mobj_t *source, mobjtype_t type)
{
    ...
    th->momz = FixedMul(getMobjInfo(th)->speed, slope);
