...
{
    return _g->midstage ? NEWTEXTSPEED :
           ((_g->midstage = _g->acceleratestage)) ? _g->acceleratestage = 0, NEWTEXTSPEED : TEXTSPEED;
}
...
void F_Ticker(void)
{
    ...
    if (!_g->finalestage)
    {
        ...
        if (_g->finalecount > strlen(_g->finaletext) * speed + (
                _g->midstage ? NEWTEXTWAIT : TEXTWAIT) || (_g->midstage && _g->acceleratestage))
        {
...
void F_CastTicker(void)
{
    ...
    if (_g->castattacking)
    {
        if (_g->castframes == 24 || _g->caststate == &states[mobjinfo[castorder[_g->castnum].type].seestate])
        {
            stopattack: _g->castattacking = false;
            _g->castframes = 0;
            _g->caststate = &states[mobjinfo[castorder[_g->castnum].type].seestate];
        }
    }
...
static void F_CastPrint(const char *text) // CPhipps - static, const char*
{
    ...
    while (ch)
    {
        ...
        w = hu_font[c]->width;
        // CPhipps - patch drawing updated
        V_DrawPatchNoScale(cx, 144, hu_font[c]);
        cx += w;
    }
}
...
void F_CastDrawer(void)
{
    ...
    sprdef = &p_wad_immutable_flash_data->sprites[_g->caststate->sprite];
    sprframe = &(getSpriteFrames(sprdef)[_g->caststate->frame & FF_FRAMEMASK]);
    lump = sprframe->lump[0];

    flip = (boolean) SPR_FLIPPED(sprframe, 0);

    // CPhipps - patch drawing updated
    V_DrawNumPatch(160, 170, 0, lump + _g->firstspritelump, CR_DEFAULT, VPT_STRETCH | (
            flip ? VPT_FLIP : 0));
}
...
// F_BunnyScroll
//
static const char pfub2[] =
{ "PFUB2" };
static const char pfub1[] =
{ "PFUB1" };
