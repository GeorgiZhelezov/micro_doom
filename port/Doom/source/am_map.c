...
static const int f_x = 0;
static const int f_y = 0;

// size of window on screen
static const int f_w = SCREENWIDTH_PHYSICAL;
static const int f_h = SCREENHEIGHT - ST_SCALED_HEIGHT;
...
#define CXMTOF(x) (f_x + MTOF((x)-_g->m_x))
#define CYMTOF(y) (f_y + (f_h - MTOF((y)-_g->m_y)))
...
static void AM_findMinMaxBoundaries(void)
{
    ...
    fixed_t max_w = (_g->max_x >>= FRACTOMAPBITS) - (_g->min_x >>= FRACTOMAPBITS); // e6y
    fixed_t max_h = (_g->max_y >>= FRACTOMAPBITS) - (_g->min_y >>= FRACTOMAPBITS); // e6y
...
{
    //  _g->f_x = _g->f_y = 0;
    //  _g->f_w = SCREENWIDTH_PHYSICAL;           // killough 2/7/98: get rid of finit_ vars
...
void AM_Start(void)
{
    ...
    printf("AM_initVariables()\r\n");
    AM_initVariables();
    printf("AM_initVariablesafter()\r\n");
}
...
static void AM_rotate(fixed_t * x, fixed_t * y, angle_t a, fixed_t xorig, fixed_t yorig)
{
    ...
    tmpx = FixedMul(*x - xorig, finecosine[a >> ANGLETOFINESHIFT]) - FixedMul(*y - yorig, finesine[a >> ANGLETOFINESHIFT]);

    *y = yorig + FixedMul(*x - xorig, finesine[a >> ANGLETOFINESHIFT]) + FixedMul(*y - yorig, finecosine[a >> ANGLETOFINESHIFT]);
    ...
    for (i = 0; i < _g->numlines; i++)
    {
        ...
        if (_g->linedata[i].r_flags & RF_MAPPED) // 2020-03-14 next-hack was ML_MAPPED, changed to RF_MAPPED
        {
            ...
            if /* jff 4/23/98 add exit lines to automap */
                (mapcolor_exit &&
                    (line_special == 11 || line_special == 52 || line_special == 197 || line_special == 51 || line_special == 124 || line_special == 198))
            {
            ...
            if (!backsector)
            {
                ...
                if (mapcolor_secr && // jff 4/3/98 0 is disable
                    ((map_secret_after && P_WasSecret(frontsector) && !P_IsSecret(frontsector)) || (!map_secret_after && P_WasSecret(frontsector))))
            ...
            else /* now for 2S lines */
            {
                ...
                else if (mapcolor_secr &&            // jff 2/16/98 fixed bug
                         (                           // special was cleared after getting it
                             (map_secret_after &&
                                 ((P_WasSecret(frontsector) && !P_IsSecret(frontsector)) ||
                                     (P_WasSecret(backsector) && !P_IsSecret(backsector)))) || // jff 3/9/98 add logic to not show secret til after entered
                             (                                                                 // if map_secret_after is true
                                 !map_secret_after && (P_WasSecret(frontsector) || P_WasSecret(backsector)))))
                {
                    AM_drawMline(&l, mapcolor_secr); // line bounding secret sector
...
static void AM_drawLineCharacter(const mline_t * lineguy, int lineguylines, fixed_t scale, angle_t angle, int color, fixed_t x, fixed_t y)
{
    ...
    if (_g->automapmode & am_rotate)
        angle -= _g->player.mo->angle - ANG90; // cph
...
static void AM_drawPlayers(void)
{
    ...
    AM_drawLineCharacter(player_arrow, NUMPLYRLINES, 0, _g->player.mo->angle, mapcolor_sngl, // jff color
...
void AM_Drawer(void)
{
    ...
    if (!(_g->automapmode & am_overlay)) // cph - If not overlay mode, clear background for the automap
    {
        ...
        V_FillRect(f_x, f_y, f_w, f_h, (byte)mapcolor_back); // jff 1/5/98 background default color
    }
