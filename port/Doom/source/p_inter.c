...
boolean P_GivePower(player_t *player, int power)
{
    ...
    { INVULNTICS, 1 /* strength */, INVISTICS, IRONTICS, 1 /* allmap */, INFRATICS, };
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
    ...
    if (delta > getMobjHeight(toucher) || delta < -8 * FRACUNIT)
        return;        // out of reach

    sound = sfx_itemup;
    player = getMobjPlayer(toucher);
...
static void P_KillMobj(mobj_t *source, mobj_t *target)
{
    ...
    // TODO:  we can exploit MF_CORPSE to reuse mobjs
    ...
    if (source && getMobjPlayer(source))
    {
        // count for intermission
        if (target->flags & MF_COUNTKILL)
            getMobjPlayer(source)->killcount++;
        if (getMobjPlayer(target))
            getMobjPlayer(source)->frags[getMobjPlayer(target) - &_g->player]++;
    }
    ...
    if (target->player_sptr)
    {
        // count environment kills against you
        if (!source)
            getMobjPlayer(target)->frags[getMobjPlayer(target) - &_g->player]++;

        target->flags &= ~MF_SOLID;
        getMobjPlayer(target)->playerstate = PST_DEAD;
        P_DropWeapon(getMobjPlayer(target));

        if (getMobjPlayer(target) == &_g->player && (_g->automapmode & am_active))
            AM_Stop();    // don't die in auto map; switch view prior to dying
    }

    if (target->health < -getMobjInfo(target)->spawnhealth && getMobjInfo(target)->xdeathstate)
        P_SetMobjState(target, getMobjInfo(target)->xdeathstate);
    else
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
    ...
    if (inflictor && !(target->flags & MF_NOCLIP ) && (!source || !getMobjPlayer(source) || getMobjPlayer(source)->readyweapon != wp_chainsaw))
    {
