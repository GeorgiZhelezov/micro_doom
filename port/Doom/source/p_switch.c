 ...
 *
 *  next-hack: switch list is now in immutable flash region (wad-dependent),
...
void P_InitSwitchList(void)
{
    ...
    episode = 1;
    short *switchlist = Z_Malloc(sizeof (*switchlist) * 2 * MAXSWITCHES, PU_STATIC, NULL);
    ...
    p_wad_immutable_flash_data->switchlist = writeBufferToFlashRegion(switchlist, sizeof(*switchlist) * 2 * MAXSWITCHES, FLASH_IMMUTABLE_REGION, true);
    Z_Free(switchlist);
}
//
// P_StartButton()
...
// No return.
//
static void P_StartButton(const line_t *line, bwhere_e w, int texture, int time)
{
    int i;
...
void P_ChangeSwitchTexture(const line_t *line, int useAgain)
{
    ...
    uint16_t *texture, *ttop, *tmid, *tbot;
    bwhere_e position;
	...
	*texture = temp_texture;
    ...
    { /* this could be more efficient... */
        ...
        else if (p_wad_immutable_flash_data->switchlist[i] == *tmid)
        {
            texture = tmid;
            position = middle;
            break;
        }
        else if (p_wad_immutable_flash_data->switchlist[i] == *tbot)
        {
            texture = tbot;
            position = bottom;
            break;
