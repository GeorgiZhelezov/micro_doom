...
{
    short tmp_y_lookup[SCREENWIDTH];    // on the stack. We have plenty.
    boolean done = false;
    int wipestart = I_GetTime() - 1;
    wipe_initMelt(tmp_y_lookup);
...
//        M_Drawer();                   // menu is drawn even on top of wipes
      ...
      // I_FinishUpdateBlock will swap buffers, but we don't want it.
      displayData.workingBuffer = 1 - displayData.workingBuffer; 
    } while (!done);
}
...
static void D_Display(void)
{
    ...
    draw_stopy = SCREENHEIGHT - ST_SCALED_HEIGHT - 1;
    draw_starty = 0;
...
static void D_UpdateFPS()
{
    ...
    unsigned int timenow = I_GetTimeMicrosecs(); // this is running at 10MHz.
...
static void D_DoomMainSetup(void)
{
    ...
    // finally fill the patch lump width array
    patchsizeoffsets_t *patchSizeOffsets = Z_Malloc( sizeof(patchsizeoffsets_t) * _g->numlumps, PU_STATIC, NULL);   
