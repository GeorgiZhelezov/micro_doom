        ...
        case 1:
            ...
            switch (direction)
            {
                case -1:
                    ...
                    // jff 2/22/98 weaken check to demo_compatibility
                    destheight =
                            (dest > sector->floorheight) ? dest : sector->floorheight;
...
void T_MoveFloor(floormove_t *floor)
{
    ...
    (getFloorMoveSector(floor), floor->speed, floor->floordestheight, floor->crush, 0, floor->direction);

    if (!(_g->leveltime & 7))     // make the floormove sound
        S_StartSound2(&getFloorMoveSector(floor)->soundorg, sfx_stnmov);
    ...
    if (res == pastdest)    // if destination height is reached
    {
        if (floor->direction == 1)       // going up
        {
            ...
            {
                case donutRaise:
                    getFloorMoveSector(floor)->special = floor->newspecial;
                    getFloorMoveSector(floor)->floorpic = floor->texture;
                    break;
                case genFloorChgT:
                case genFloorChg0:
                    getFloorMoveSector(floor)->special = floor->newspecial;
                    //jff add to fix bug in special transfers from changes
                    getFloorMoveSector(floor)->oldspecial = floor->oldspecial;
                    // fall through
                case genFloorChg:
                    getFloorMoveSector(floor)->floorpic = floor->texture;
                    break;
        ...
        getFloorMoveSector(floor)->floordata_sptr = 0; //jff 2/22/98
        P_RemoveThinker(&floor->thinker); //remove this floor from list of movers

        // make floor stop sound
        S_StartSound2(&getFloorMoveSector(floor)->soundorg, sfx_pstop);
    }
}
...
void T_MoveElevator(elevator_t *elevator)
{
    ...
    {
        elevator->sector->floordata_sptr = 0;     //jff 2/22/98
        elevator->sector->ceilingdata_sptr = 0;   //jff 2/22/98
...
int EV_DoFloor(const line_t *line, floor_e floortype)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        sec->floordata_sptr = getShortPtr(floor); //jff 2/22/98
        ...
        switch (floortype)
        {
            case lowerFloor:
                floor->direction = -1;
                floor->sector_sptr = getShortPtr(sec);
            ...
            case lowerFloor24:
                ...
                floor->sector_sptr = getShortPtr(sec);
                floor->speed = FLOORSPEED;
                floor->floordestheight = getFloorMoveSector(floor)->floorheight + 24 * FRACUNIT;
                break;
            ...
            case lowerFloor32Turbo:
                ...
                floor->floordestheight = getFloorMoveSector(floor)->floorheight + 32 * FRACUNIT;
                break;
            ...
            case lowerFloorToNearest:
                ...
                floor->sector_sptr = getShortPtr(sec);
                floor->speed = FLOORSPEED;
                floor->floordestheight = P_FindNextLowestFloor(sec, getFloorMoveSector(floor)->floorheight);
                break;
            ...
            case raiseFloor512:
                ...
                floor->sector_sptr = getShortPtr(sec);
                floor->speed = FLOORSPEED;
                floor->floordestheight = getFloorMoveSector(floor)->floorheight + 512 * FRACUNIT;
                break;
            ...
            case raiseToTexture:
            {
                ...
                {
                    floor->floordestheight = (getFloorMoveSector(floor)->floorheight >> FRACBITS) + (minsize >> FRACBITS);
 ...
 */
static inline int P_FindSectorFromLineTagWithLowerBound(const line_t *l, int start, int min)
{
...
int EV_BuildStairs(const line_t *line, stair_e type)
{
    ...
    while ((ssec = P_FindSectorFromLineTagWithLowerBound(line, ssec, minssec)) >= 0)
    {
        ...
        { //jff 2/22/98
            ...
            memset(floor, 0, sizeof(*floor));
            P_AddThinker(&floor->thinker);
            sec->floordata_sptr = getShortPtr(floor);
            floor->thinker.function = T_MoveFloor;
            floor->direction = 1;
            floor->sector_sptr = getShortPtr(sec);
...
//    s2 = getNextSector(s1->lines[0],s1);  // s2 is pool's sector
        s2 = getNextSector(getSectorLineByIndex(s1, 0), s1); // s2 is pool's sector
        if (!s2)
        ...
        for (i = 0; i < s2->linecount; i++)
        {
            ...
            if (!LN_BACKSECTOR((getSectorLineByIndex(s2, i))) || LN_BACKSECTOR((getSectorLineByIndex(s2,i))) == s1)
                continue;

            rtn = 1; //jff 1/26/98 no donut action - no switch change on return

            s3 = LN_BACKSECTOR((getSectorLineByIndex(s2, i))); // s3 is model sector for changes

            //  Spawn rising slime
            floor = Z_Malloc(sizeof(*floor), PU_LEVSPEC, 0);
            memset(floor, 0, sizeof(*floor));
            P_AddThinker(&floor->thinker);
            s2->floordata_sptr = getShortPtr(floor); //jff 2/22/98
            floor->thinker.function = T_MoveFloor;
            floor->type = donutRaise;
            floor->crush = false;
            floor->direction = 1;
            floor->sector_sptr = getShortPtr(s2);
            ...
            memset(floor, 0, sizeof(*floor));
            P_AddThinker(&floor->thinker);
            s1->floordata_sptr = getShortPtr(floor); //jff 2/22/98
            floor->thinker.function = T_MoveFloor;
            floor->type = lowerFloor;
            floor->crush = false;
            floor->direction = -1;
            floor->sector_sptr = getShortPtr(s1);
            floor->speed = FLOORSPEED / 2;
            floor->floordestheight = s3->floorheight;
            break;
        }
...
//
int EV_DoElevator(const line_t *line, elevator_e elevtype)
{
    int secnum;
    int rtn;
    sector_t *sec;
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        if (sec->floordata_sptr || sec->ceilingdata_sptr) //jff 2/22/98
            continue;
        ...
        sec->floordata_sptr = getShortPtr(elevator); //jff 2/22/98
        sec->ceilingdata_sptr = getShortPtr(elevator); //jff 2/22/98
